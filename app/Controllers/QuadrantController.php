...
class QuadrantController extends BaseController
{
    ...
    public function getQuadrant2()
    {
        ...
        $mailset['FromTIme'] = isset($this->request->getGet()['FromTIme']) ? $this->request->getGet()['FromTIme'] : $mailset['FromTIme'];
        $mailset['ToTIme'] = isset($this->request->getGet()['ToTIme']) ? $this->request->getGet()['ToTIme'] : $mailset['ToTIme'];
        $mailset['creditMultiplier'] = isset($this->request->getGet()['creditMultiplier']) ? $this->request->getGet()['creditMultiplier'] : $mailset['labelCreditMultiplier'];
        $array = $pick->getAllCasesPallets($displayDate, $type, $mailset['docType'], $cust);
        ...
        $pickersSummary = [];
        foreach ($times as $time):
            $table_head .= "<th colspan='2'>" . $time['start'] . "</th>";
            $timehead = array('time' => $time['start'], 'count' => 0, 'time_top_right' => 0, 'time_top_left' => 0, 'time_bottom_right' => 0, 'time_bottom_left' => 0 );
            array_push($timeheads, $timehead);
        endforeach;
        $table_head .= "<th colspan='2'>Picker Total</th>";
        $table_head .= "</tr></thead>";
        $table_body = "<tbody>";
        $pickers = [];
        foreach ($array as $item):
        ...
        foreach ($pickers as $picks):
            $table_body .= "<tr><td rowspan='2'>" . $picks . "</td>";
            $time_count = 0;
            $picker_top_left = 0;
            $picker_top_right = 0;
            $picker_bottom_left = 0;
        ...
        endforeach;
        //$table_body .= "</tbody>";
        $table_foot = "<tr><td rowspan='2'>Time Total</td>";
        $total_top_left = 0;
        $total_top_right = 0;
        $total_bottom_left = 0;
        $total_bottom_right = 0;
